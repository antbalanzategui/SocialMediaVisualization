...
public class Input {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for (int i = 0; i < man.getList().toArray().length; i++) {
            int janTotalEng = 0;
            int marchTotalEng = man.getList().getEntry(i).getEngagementForMonth(
                MonthEnum.MARCH).getTotalEngagement();

            int febTotalEng = man.getList().getEntry(i).getEngagementForMonth(
                MonthEnum.FEBRUARY).getTotalEngagement();

            if (man.getList().getEntry(i).getEngagementForMonth(
                MonthEnum.JANUARY) != null) {
                janTotalEng = man.getList().getEntry(i).getEngagementForMonth(
                    MonthEnum.JANUARY).getTotalEngagement();
            }
        ...
        for (int i = 0; i < man.getList().toArray().length; i++) {
            ...
            if (man.getList().getEntry(i).getEngagementForMonth(
                MonthEnum.JANUARY) != null) {
                janTotalEng1 = man.getList().getEntry(i).getEngagementForMonth(
                    MonthEnum.JANUARY).getTotalEngagement();
            }

            int marchTotalViews = man.getList().getEntry(i)
                .getEngagementForMonth(MonthEnum.MARCH).getNumViews();

            int febTotalViews = man.getList().getEntry(i).getEngagementForMonth(
                MonthEnum.FEBRUARY).getNumViews();

            if (man.getList().getEntry(i).getEngagementForMonth(
                MonthEnum.JANUARY) != null) {
                janTotalViews = man.getList().getEntry(i).getEngagementForMonth(
                    MonthEnum.JANUARY).getNumViews();
            }
